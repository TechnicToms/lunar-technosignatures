...
from models.cp_transformer import ViT
...
class CutAndPasteViT(pl.LightningModule):
    def __init__(self, img_size: int, patch_size: int, cut_patch_size: int):
        super().__init__()
        self.transformer = ViT(img_size=img_size, patch_size=patch_size)
        
        self.transformations = [CutPasteTransform(patch_size=cut_patch_size, use_flipping=True),
                                CutPatchMeanTransform(patch_size=cut_patch_size), WhitenBlobTransform(width=(3, 8)),
...
class CutAndPasteDataLoader(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        dataset = constructApollo17DatasetWOApollo(self.root, self.imgSize, self.stride)
...
if __name__ == "__main__":
    ...
    # logger = TensorBoardLogger("tb_logs", name="CutAndPaste", version="Apollo17")
